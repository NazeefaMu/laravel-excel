...
class Product extends Model
{
    ...
    protected $fillable = ['sku','store_view_code','colour','size','bar_code','b2c','brand','attribute_set_code','product_type','categories','product_websites','group_name',
            'description','hort_description','weight','product_online','tax_class_name','visibility','price','special_price','special_price_from_date','special_price_to_date',
            ...
            'max_cart_qty','use_config_max_sale_qty','in_stock','notify_on_stock_below','use_config_notify_stock_qty','manage_stock',
            ...
            'downloadable_links','downloadable_samples','configurable_variations','configurable_variation_labels','domain_id'];
    ...
    public static function getProducts(){
        $records= DB::table('product')->select('id','sku','description','colour','size','group_name','bar_code','in_stock','b2c','brand','product_type'
            ...
            'option2_value','option3_name','option3_value','barcode','domain_id')->get()->toArray();
        return $records;
    }

    public static function getMagentoProducts(){
        $records=DB::table('product')->select( 'sku','store_view_code','attribute_set_code','product_type','categories','product_websites','group_name',
            'description','hort_description','weight','product_online','tax_class_name','visibility','price','special_price','special_price_from_date','special_price_to_date',
            'url_key','meta_title','meta_keywords','meta_description','base_image','base_image_label','small_image','small_image_label',
            'thumbnail_image','thumbnail_image_label','swatch_image','swatch_image_label','created_at','updated_at','new_from_date','new_to_date',
            'display_product_options_in','map_price','msrp_price','map_enabled','gift_message_available','custom_design',
            ...
            'upsell_position','additional_images','additional_image_labels','hide_from_product_page','custom_options','bundle_price_type',
            'bundle_sku_type','bundle_price_view','bundle_weight_type','bundle_values','bundle_shipment_type','associated_skus',
            'downloadable_links','downloadable_samples','configurable_variations','configurable_variation_labels','domain_id')->get()->toArray();
        return $records;
    }
